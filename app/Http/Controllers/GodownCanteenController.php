...
use App\Models\User;
use App\Models\Item;
use App\Models\GodownCanteenItem;
...
class GodownCanteenController extends Controller {
    ...
    public function dailyEntries(Request $request){
        return view('admin.canteens_godowns.daily_entries.index', [
    ...
    public function editCanteenItem(Request $request){
        ...
        $canteen_item = GodownCanteenItem::find($request->canteen_item_id);
    ...
    public function storeCanteenItem(Request $request){
        ...
        if($request->has('id')){
            ...
            $c_item = GodownCanteenItem::where('barcodevalue', $request->barcodevalue)->where('client_id', $client_id)->first();
            if($c_item){
        ...
        if($validator->passes()){
            ...
            $canteen_item = GodownCanteenItem::find($request->id);
            $data['message'] = 'This item is updated successfully to the canteen';
            if(!$canteen_item){
                $canteen_item = new GodownCanteenItem;
    ...
    public function initCanteenItemsDrop(Request $request){
        ...
        $canteen_items = DB::table('g_canteen_items')->select('id as canteen_item_id','price','item_name','barcodevalue')->where('status',0)->where('stock','>',0)->where('client_id',$client_id)->get();
    ...
    public function initCanteenItemStocks (Request $request){
        ...
        $item_stocks = DB::table('g_canteen_item_stocks')->where('g_canteen_item_stocks.canteen_item_id',$canteen_item_id)->where('g_canteen_item_stocks.client_id',$client_id)->orderBy('id','DESC')->get();
    ...
    public function editCanteenItemStocks(Request $request){
        ...
        $item_stock = DB::table('g_canteen_item_stocks')->where('id',$canteen_item_stock_id)->first();
    ...
    public function storeCanteenItemStock(Request $request){
        ...
        if($validator->passes()){
            ...
            if($request->id){  
                ...
                $canteen_item_stock = DB::table('g_canteen_item_stocks')->where('id',$request->id)->first();
                $added_stock = $canteen_item->stock  - $canteen_item_stock->stock;
            
                DB::table('g_canteen_item_stocks')->where('id',$request->id)->update($ins_data);
                $data['message'] = "Successfully Updated";
            }else{
                $ins_data['date'] = date("Y-m-d");
                $ins_data['created_at'] = date("Y-m-d H:i:s");
                DB::table('g_canteen_item_stocks')->insert($ins_data);
    ...
    public function initEntries(Request $request){
        ...
        $canteen_items = DB::table('g_canteen_items')->select('id as canteen_item_id','price','item_name','barcodevalue')->where("stock", '>', 0)->where('client_id',$client_id)->get();
    ...
    public function editEntry(Request $request){
        ...
        if($s_entry){
            ...
            $products = DB::table('g_daily_entry_items')->select('g_daily_entry_items.*','canteen_items.id','canteen_items.price','canteen_items.item_name')->leftJoin('canteen_items','canteen_items.id','=','g_daily_entry_items.canteen_item_id')->where('g_daily_entry_items.entry_id','=',$s_entry->id)->get();
    ...
    public function store(Request $request){
        ...
        foreach ($final_items as $key => $item) {
            if($item['quantity'] !=0){
                DB::table('g_daily_entry_items')->insert([
                    'canteen_item_id' => $item['canteen_item_id'],
                    'entry_id' => $entry_id,
                ...
                $check = DB::table('g_canteen_items')->where('id',$item['canteen_item_id'])->first();
                $avil_stock = $check->stock;
                DB::table('g_canteen_items')->where('id',$item['canteen_item_id'])->update([
    ...
    public function printBill($entry_id=0){
        ...
        $s_entry->total_quantity = DB::table('g_daily_entry_items')->where('g_daily_entry_items.entry_id','=',$s_entry->id)->sum('quantity');
    ...
    public function printBarcode($id=0){
        $item = DB::table('g_canteen_items')->where('id',$id)->first();
        if($item){
            return view('admin.canteens_godowns.canteen_items.barcode',compact('item'));
    ...
    public function setBarcode(){
        ...
        foreach ($items as $key => $item) {
            $check = DB::table("g_canteen_items")->where('barcodevalue',$barcode)->first();
            if($check){
                $barcode = $barcode+30;
            }
            DB::table('g_canteen_items')->where('id',$item->id)->update([
    ...
    public function printItemsBarcode(){
        ...
        $items = DB::table('canteens_godowns')->where('is_manual',1)->where('client_id',Auth::user()->client_id)->get();
        

        $pdf = Pdf::loadView('admin.canteens_godowns.canteen_items.mbarcode',compact('items'));
