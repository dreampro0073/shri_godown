...
@section('main')
    <div class="main" ng-controller="gCanteenItemsCtrl" ng-init="init();"> 
        @include("admin.canteens_godowns.canteen_items.add")
                ...
                <table class="table table-bordered table-striped" >
                    ...
                    <tbody ng-if="canteen_items.length > 0">
                        <tr ng-repeat="item in canteen_items">
                            ...
                            <td>@{{ item.price }}</td>
                            <td>@{{ item.stock }} <a href="{{url('admin/godown-canteen/items/stock/')}}/@{{item.id}}" style="margin-left: 20px;text-decoration: underline;">View Stock</a></td>
                            <td>
                                <a href="javascript:;" class="btn btn-sm btn-warning" ng-click="edit(item.id)">Edit</a>
                                <a target="_blank" href="{{url('admin/godown-canteen/items/print-barcode/')}}/@{{item.id}}" class="btn btn-sm btn-warning">Print Barcode</a>
                            </td>
